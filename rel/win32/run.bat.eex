...
REM msg "%username%" %ARGS%
if "%RELEASE_COMMAND%" == "" (set RELEASE_COMMAND=start)
...
if "%RELEASE_COMMAND%" == "start" (set "REL_EXEC=elixir" && set "REL_EXTRA=--no-halt" && set "REL_GOTO=start")
if "%RELEASE_COMMAND%" == "start_iex" (set "REL_EXEC=iex" && set "REL_EXTRA=--werl" && set "REL_GOTO=start")
if "%RELEASE_COMMAND%" == "install" (set "REL_GOTO=install")
if "%RELEASE_COMMAND%" == "eval" (
  if "%~2" == "" (
...
if "%RELEASE_COMMAND%" == "version" (goto version)
if "%RELEASE_COMMAND%" == "stop" (set "REL_RPC=System.stop()" && goto rpc)
if "%RELEASE_COMMAND%" == "restart" (set "REL_RPC=System.restart()" && goto rpc)
if "%RELEASE_COMMAND%" == "pid" (set "REL_RPC=IO.puts(System.pid())" && goto rpc)
...
echo.
if not "%RELEASE_COMMAND%" == "" (echo ERROR: Unknown command %~1)
goto end
